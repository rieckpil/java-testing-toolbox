...
public class ImageUploadEventListener {
  ...
  public void processImageUploadEvent(ImageUploadEvent event) throws IOException {
    ...
    s3Client.getObject(GetObjectRequest.builder().bucket(event.getS3Bucket()).key(event.getS3Key()).build(), ResponseTransformer.toBytes())
      .asInputStream()
      .transferTo(new FileOutputStream(fileToBeResized));
